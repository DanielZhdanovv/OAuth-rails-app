...
RSpec.describe Client::SessionsController, type: :controller do # rubocop:disable Metrics/BlockLength
  include WebMock::API
  ...
  describe 'GET #callback' do # rubocop:disable Metrics/BlockLength
    ...
    context 'with valid state' do
      ...
      it 'returns success' do
        stub_request(:post, 'http://localhost:3000/server/oauth/token')
          .with(
            body: { 'client_id' => 'client_app_123', 'code' => 'test_code_1234', 'code_verifier' => 'code_verifier',
    ...
    context 'with invalid state' do
      ...
      it 'returns error' do
        subject
        expect(response.parsed_body).to eq({ 'error' => 'Invalid state' })
      end
    end
  end
  describe 'GET #refresh_tokens' do # rubocop:disable Metrics/BlockLength
    ...
    context 'with valid params' do
      ...
      it 'returns success' do
        stub_request(:post, 'http://localhost:3000/server/oauth/token')
          .with(
            body: { 'client_id' => 'client_app_123', 'grant_type' => 'refresh_token',
