        ...
        context 'with valid access token' do
            ...
            it 'returns user info' do
                stub_request(:get, "http://localhost:3000/server/oauth/user").
                    ...
                    to_return(status: 200, body: { "first_name" => "Adam", "last_name" => "Smith", "email" => "adam.smith@example.com" }.to_json, headers: {})
                    subject
                    expect(response).to have_http_status(:success)
                    expect(JSON.parse(response.body)).to eq({ "first_name" => "Adam", "last_name" => "Smith", "email" => "adam.smith@example.com" })
            end
        end
        context 'with invalid access token' do
            ...
            it 'returns error' do
                stub_request(:get, "http://localhost:3000/server/oauth/user").
                    ...
                    expect(JSON.parse(response.body)).to eq({ "error" => "Error fetching user info" })
            end
        end
    end
end
